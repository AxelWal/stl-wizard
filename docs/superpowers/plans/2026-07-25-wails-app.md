...
`wails doctor` reports missing system dependencies. On Linux you need `webkit2gtk` development headers — on Debian/Ubuntu `libwebkit2gtk-4.0-dev` (or `-4.1-dev`), on Arch `webkit2gtk`. Install whatever it names before continuing; nothing later in this plan works without it.
...
Requires Go 1.26 and the Wails v2 CLI:
    ...
    go install github.com/wailsapp/wails/v2/cmd/wails@v2.13.0
    wails doctor      # install whatever system dependencies it names
    wails dev
...
```bash
go vet ./... && go test ./... -race && wails build
```
